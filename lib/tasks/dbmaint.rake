namespace :dbmaint do
  ...
  task import_from_heroku: :environment do
    HEROKU_APP_NAME = 'core-revival' # Change this if app name is not picked up by `heroku` git remote.
    # c = Rails.configuration.database_configuration[Rails.env]
    heroku_app_flag = HEROKU_APP_NAME ? " --app #{HEROKU_APP_NAME}" : nil
    Bundler.with_clean_env do
      puts "[1/5] make sure you have run: 'createdb core_revival_dev'"
      ...
      `pg_restore --clean --verbose --no-acl --no-owner -h localhost -d core_revival_dev tmp/latest.dump`
